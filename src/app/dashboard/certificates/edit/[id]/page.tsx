  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      try {
        ...
        const cert = data.certificates?.find((c: any) => c.original_id === parseInt(id));
        
        if (cert) {
          // Un-wrap wsrv.nl proxy if it exists so the input field shows the clean url
          let url = cert.file_url;
          if (url && url.includes('wsrv.nl/?url=')) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold font-outfit text-slate-900">Edit Sertifikat</h2>
          <p className="text-slate-500 text-sm">Perbarui detail sertifikat Anda.</p>
        </div>
      </div>
